...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">{{ $workOrder->product_name }}</p>
        </div>
        <div class="flex items-center gap-3">
            @if($workOrder->status === 'allocated')
                <form method="POST" action="{{ route('craftsman.work-orders.accept', $workOrder) }}">
                    @csrf
                    <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg shadow-sm">
                        Accept Work Order
                    </button>
                </form>
            @elseif(in_array($workOrder->status, ['in_process', 'returned']))
            ...
            @endif
            <a href="{{ $backUrl }}" class="px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50">Back</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sm:p-8">
        <div class="flex flex-col sm:flex-row gap-6">
            <div class="sm:w-1/3">
                @if($workOrder->design_image)
                    <img src="{{ asset('storage/' . $workOrder->design_image) }}" class="w-full h-56 object-cover rounded-xl border border-slate-200 shadow-sm">
                @else
                    <div class="w-full h-56 rounded-xl bg-slate-50 border border-dashed flex items-center justify-center text-slate-400 text-xs">No Design Image</div>
                @endif
            </div>

            <div class="sm:w-2/3 grid grid-cols-2 gap-4 text-sm">
                <div>
                    <span class="text-xs text-slate-400 uppercase font-semibold">Design Code</span>
                    <p class="font-mono font-bold text-indigo-600 text-base">{{ $workOrder->design_code }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 uppercase font-semibold">Gold Hallmark / Purity</span>
                    <p class="font-bold text-slate-800">{{ $workOrder->hallmark_purity }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 uppercase font-semibold">Target Metal Weight</span>
                ...
                <div>
                    ...
                    <p class="font-semibold text-slate-800">{{ $workOrder->quantity }} {{ $workOrder->unit_type }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 uppercase font-semibold">Screw Type</span>
                    <p class="font-medium text-slate-800">{{ $workOrder->screw_type ?? 'None / N/A' }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 uppercase font-semibold">Rhodium Polish</span>
                    <p class="font-semibold {{ $workOrder->rhodium_polish ? 'text-emerald-600' : 'text-slate-500' }}">{{ $workOrder->rhodium_polish ? 'Required (Yes)' : 'No' }}</p>
                </div>
            </div>
        </div>
        ...
        @if($workOrder->instructions)
            ...
            <div>
                ...
                <p class="text-sm text-slate-800 mt-1 bg-amber-50/50 p-4 rounded-lg border border-amber-100 leading-relaxed">{{ $workOrder->instructions }}</p>
            </div>
        @endif
    </div>
